              ...
              {filteredLogs.map((log) => (
                <tr key={log.id} className="hover:bg-slate-50 transition">
                  <td className="px-4 py-3 font-medium text-slate-900">
                    <div>{log.metadata?.customerName || 'Customer'}</div>
                  ...
                  <td className="px-4 py-3 text-slate-700">
                    <div>{log.metadata?.reservationName || 'Sovereign Land'}</div>
                    <div className="text-[10px] text-slate-500">{log.metadata?.state || 'US'}</div>
                  </td>
